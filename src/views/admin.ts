...
<body class="bg-gray-900 text-white min-h-screen">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex justify-between h-16">
        <div class="flex items-center">
          ...
          <span class="ml-4 px-3 py-1 bg-red-900/50 text-red-300 rounded-full text-sm">管理后台</span>
        </div>
        <div class="flex items-center space-x-4">
          <a href="/" class="px-4 py-2 hover:text-red-400 transition">返回首页</a>
          <a href="/profile" class="px-4 py-2 hover:text-red-400 transition">个人中心</a>
  ...
  <div class="flex">
    <aside class="w-64 bg-gray-800 min-h-screen p-6">
      <nav class="space-y-2">
        <a href="/admin" class="block px-4 py-3 rounded-lg hover:bg-gray-700 transition ${title === '仪表盘' ? 'bg-red-600' : ''}">
          📊 仪表盘
        </a>
        <a href="/admin/movies" class="block px-4 py-3 rounded-lg hover:bg-gray-700 transition ${title === '影视管理' ? 'bg-red-600' : ''}">
          🎬 影视管理
        </a>
        <a href="/admin/users" class="block px-4 py-3 rounded-lg hover:bg-gray-700 transition ${title === '用户管理' ? 'bg-red-600' : ''}">
          👥 用户管理
        </a>
      </nav>
    </aside>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
      <div class="bg-gray-800 rounded-xl p-6 shadow-lg">
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-gray-400">影视总数</h3>
      ...
      <div class="bg-gray-800 rounded-xl p-6 shadow-lg">
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-gray-400">用户总数</h3>
          <span class="text-4xl">👥</span>
        </div>
          ...
          <tbody>
            ${stats.recentMovies.slice(0, 5).map(movie => `
              <tr class="border-b border-gray-700/50 hover:bg-gray-700/50">
                ...
                <td class="py-3 px-4 text-yellow-400">★ ${movie.rating}</td>
                <td class="py-3 px-4">
                  <a href="/admin/movies/edit/${movie.id}" class="text-red-500 hover:text-red-400">编辑</a>
                </td>
              </tr>
            `).join('')}
          </tbody>
...
export function moviesListPage(movies: Movie[]): string {
  const content = `
    <div class="flex justify-between items-center mb-8">
      ...
      <a href="/admin/movies/create" class="px-6 py-3 bg-red-600 hover:bg-red-700 rounded-lg transition">
        + 添加新影视
      </a>
    </div>
          ...
          <tbody>
            ${movies.map(movie => `
              <tr class="border-b border-gray-700/50 hover:bg-gray-700/50 transition">
                <td class="py-4 px-6 text-gray-400">${movie.id}</td>
                <td class="py-4 px-6 font-semibold">${movie.title}</td>
                <td class="py-4 px-6">${movie.year}</td>
                <td class="py-4 px-6">
                  <div class="flex flex-wrap gap-1">
                    ${movie.genre.slice(0, 2).map(g => `<span class="px-2 py-1 bg-red-900/50 text-red-300 rounded text-xs">${g}</span>`).join('')}
                  </div>
                </td>
                <td class="py-4 px-6 text-yellow-400">★ ${movie.rating}</td>
                <td class="py-4 px-6">${movie.duration}分钟</td>
                <td class="py-4 px-6">
                  <a href="/admin/movies/edit/${movie.id}" class="text-blue-400 hover:text-blue-300 mr-3">编辑</a>
                  <button onclick="confirmDelete(${movie.id}, '${movie.title}')" class="text-red-400 hover:text-red-300">删除</button>
                </td>
              </tr>
            `).join('')}
          </tbody>
...
export function movieFormPage(movie?: Movie, error?: string): string {
  ...
  const content = `
    <div class="max-w-4xl">
      ...
      ${error ? `
        <div class="bg-red-900/50 border border-red-500 text-red-300 px-4 py-3 rounded mb-6">
          ${error}
        </div>
      ` : ''}
      ...
      <form action="/admin/movies/${isEdit ? 'update' : 'create'}" method="POST" class="bg-gray-800 rounded-xl p-8 shadow-lg">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div class="md:col-span-2">
            ...
            <input type="text" name="cast" id="cast" required placeholder="演员1, 演员2, 演员3" value="${movie?.cast.join(', ') || ''}"
                   class="w-full px-4 py-3 bg-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500">
          </div>
          ...
          <tbody>
            ${users.map(user => `
              <tr class="border-b border-gray-700/50 hover:bg-gray-700/50 transition">
                ...
                <td class="py-4 px-6">
                  <span class="px-3 py-1 rounded ${user.role === 'admin' ? 'bg-red-900/50 text-red-300' : 'bg-blue-900/50 text-blue-300'}">
                    ${user.role === 'admin' ? '管理员' : '普通用户'}
                  </span>
                </td>
                <td class="py-4 px-6">${user.createdAt.toLocaleDateString('zh-CN')}</td>
                <td class="py-4 px-6">${user.lastLogin ? user.lastLogin.toLocaleDateString('zh-CN') : '从未登录'}</td>
              </tr>
            `).join('')}
          </tbody>
        </table>
